...
use anchor_types::KlipperCommandFlags;
...
pub struct Command {
	...
	// pub flags: u32, // KlipperCommandFlags as u32
	pub flags: KlipperCommandFlags, // KlipperCommandFlags as u32
}
...
impl Command {
	...
	pub fn get_desc_string(&self) -> String {
		build_message_descriptor(
			...
			self.args
				.iter()
				.map(|a| DescArg { name: &a.name, type_: &a.type_ }),
		)
	}
}

fn parse_has_context_param<'a>(iter: &mut std::iter::Peekable<impl Iterator<Item = (usize, &'a syn::FnArg)>>) -> bool {
...
impl Parse for Command {
	fn parse(input: ParseStream) -> Result<Self> {
		...
		for (idx, arg) in inputs {
			match arg {
				syn::FnArg::Typed(PatType { pat, colon_token: Colon { .. }, ty, .. }) => match pat.as_ref() {
		...
		Ok(Command {
			...
			args,
			error_type,
			flags: KlipperCommandFlags::empty(), // Default: no flags
		})
	}
}
