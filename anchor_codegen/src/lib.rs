...
impl Processor {
	...
	fn queue_submodule(&mut self, name: &Ident) -> Result<()> {
		...
		let candidates: Vec<_> = [base.join(format!("{}.rs", name)), base.join(name.to_string()).join("mod.rs")]
			.into_iter()
			.filter(|p| p.exists())
			.collect();

		let file = match candidates.len() {
			2 => panic!("Both {}.rs and {}/mod.rs exist. Remove one to break ambiguity.", name, name),
			0 => panic!("Cannot find either {}.rs or {}/mod.rs", name, name),
			1 => &candidates[0],
			_ => unreachable!(),
		};
		self.queue
			.push_back(Task { path: file.to_owned(), module_path });
		Ok(())
	}
	...
	fn convert_id(id: u16) -> i16 {
		...
		// so the host and MCU agree on the exact bytes for this id.
		let sv = id as i32;
	...
	fn finalize_dictionary(&mut self) {
		...
		self.dictionary
			.enumerations
			.insert("static_string_id".to_string(), DictionaryEnumeration(static_string_enum));
	}
		...
		self.messages
			...
			.map(|m| match m {
				Message::Command(c) => {
					...
					let target = c.target();
					let ctx_arg = c.has_context.then(|| {
						quote! {
							context,
						}
					});
