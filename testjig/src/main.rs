...
impl KlipperInstance {
	fn new(cfg: impl AsRef<str>) -> Self {
		let klipper_path = env::var("KLIPPER_PATH").expect("set `KLIPPER_PATH` environment variable to klipper path");
		...
		{
			let mut cfg_file = std::fs::File::create(&cfg_filename).expect("Could not open config file");
			cfg_file
		...
		KlipperInstance { _temp_dir: temp_dir, child, socket_path }
	}
...
impl SerialEmulator {
	fn new() -> Self {
		...
		SerialEmulator { master: ptys.master, slave: ptys.slave }
	}
